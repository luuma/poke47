...
int ProcessPlayerFieldInput(struct FieldInput *input)
{
    ...
    if ((gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_BIKE) && input->pressedAButton && TryDingBell())
        return TRUE;
...
static const u8 *GetInteractedBackgroundEventScript(struct MapPosition *position, u8 metatileBehavior, enum Direction direction)
{
    ...
    case BG_EVENT_HIDDEN_ITEM:
        ...
        if (FlagGet(FLAG_GAUNTLET_CHALLENGE) == TRUE)
            gSpecialVar_0x8005 = GauntletReturnRngMint();// random mint
...
static bool32 TryDingBell()
{
    //if (!IsSEPlaying()) objectively funnier
    {
        PlaySE(SE_BIKE_BELL);// ScriptContext_SetupScript(EventScript_DINGDING);
        return FALSE;
    }
    return FALSE;
}
...
static bool8 TryStartStepCountScript(u16 metatileBehavior)
{
    ...
    if (!(gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_FORCED_MOVE) && !MetatileBehavior_IsForcedMovementTile(metatileBehavior))
    {
        ...
        if (ShouldBecomeVisible() == TRUE)
        {
            ScriptContext_SetupScript(EventScript_BecomeVisible);
            return TRUE;
        }
