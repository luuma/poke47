...
{
    u16 rnd = GetMirageRnd() >> 30;
    int i;

    for (i = 0; i < PARTY_SIZE; i++)
        if (GetMonData(&gParties[B_TRAINER_PLAYER][i], MON_DATA_SPECIES) && (GetMonData(&gParties[B_TRAINER_PLAYER][i], MON_DATA_PERSONALITY) & 0b111111) == rnd) //look harder, old man
            return TRUE;
