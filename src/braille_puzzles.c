...
bool8 CheckRelicanthWailord(void)
{
    ...
    // First comes Wailord
    if (GetMonData(&gParties[B_TRAINER_PLAYER][0], MON_DATA_SPECIES_OR_EGG, 0) == SPECIES_LUMINEON)
    {
        CalculatePlayerPartyCount();
        // Last comes Relicanth
        if (GetMonData(&gParties[B_TRAINER_PLAYER][gPartiesCount[B_TRAINER_PLAYER] - 1], MON_DATA_SPECIES_OR_EGG, 0) == SPECIES_SIGILYPH)
            return TRUE;
    }
    return FALSE;
}
