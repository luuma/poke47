...
u32 GetSoftLevelCapExpValue(u32 level, u32 expValue)
{
    ...
    static const u32 sExpGauntletScalingUp[4]   = {4, 2, 1};
    u32 levelDifference;
    ...
    if (FlagGet(FLAG_GAUNTLET_CHALLENGE))
    {
        if (level < 16)
        {
            levelDifference = 16 - level;
            if (levelDifference > ARRAY_COUNT(sExpGauntletScalingUp) - 1)
                return expValue *3;// default to triple
            else
                return expValue*2 + (expValue / sExpGauntletScalingUp[levelDifference]);
        }
        return expValue*2;// always double
    }
