...
static enum CancelerResult CancelerMoveFailure(struct BattleCalcValues *cv)
{
    ...
    case EFFECT_ENDURE:
        ...
        break;
    case EFFECT_REST:
        if (gBattleMons[cv->battlerAtk].status1 & STATUS1_SLEEP)//         || ctx->abilityAtk == ABILITY_COMATOSE
...
{
    if ((!IsDanceMove(cv->move) && !IsSoundMove(cv->move))
    ...
    for (enum BattlerId battler = 0; battler < gBattlersCount; battler++)
    {
        ...
        if (cv->abilities[battler] == ABILITY_DANCER || cv->abilities[battler] == ABILITY_PARROT)
...
static enum MoveEndResult MoveEndDancer(struct BattleCalcValues *cv)
{
    ...
    if (AbilityBattleEffects(ABILITYEFFECT_DANCER, cv->battlerAtk, ABILITY_DANCER, cv->move, TRUE)|| AbilityBattleEffects(ABILITYEFFECT_DANCER, cv->battlerAtk, ABILITY_PARROT, cv->move, TRUE))
        result = MOVEEND_RESULT_RUN_SCRIPT;
