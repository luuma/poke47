...
static bool8 DisplayPartyPokemonDataForMoveTutorOrEvolutionItem(u8 slot)
{
    ...
    else
    {
        ...
        case ITEM_IS_EVOLUTION_STONE: // Evolution stone
            if (!GetMonData(currentPokemon, MON_DATA_IS_EGG) && GetEvolutionTargetSpecies(currentPokemon, EVO_MODE_ITEM_CHECK, item, NULL, NULL, CHECK_EVO, FALSE) != SPECIES_NONE)
                return FALSE;
...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc task)
{
    ...
    if (cannotUseEffect)
    {
        ...
        {
            targetSpecies = GetEvolutionTargetSpecies(mon, EVO_MODE_NORMAL, ITEM_NONE, NULL, &canStopEvo, CHECK_EVO, FALSE);
        }

        if (targetSpecies != SPECIES_NONE)
        {
            GetEvolutionTargetSpecies(mon, EVO_MODE_NORMAL, ITEM_NONE, NULL, &canStopEvo, DO_EVO, FALSE);
            RemoveBagItem(gSpecialVar_ItemId, 1);
            FreePartyPointers();
