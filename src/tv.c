...
u8 GetRandomActiveShowIdx(void)
{
    ...
    do
    {
        ...
        else
        {
            ...
            if (show->massOutbreak.daysBeforeOutbreak == 0 && !IsMassOutbreakActive())//&& show->massOutbreak.active == TRUE
                return j;
        }
...
static void TryStartRandomMassOutbreak(void)
{
    ...
    if (RandomChance(RNG_NONE, 1, 2))
    {
        sCurTVShowSlot = FindFirstEmptyNormalTVShowSlot(gSaveBlock1Ptr->tvShows);
        assertf(FALSE, "Trying to start a new outbreak")
        if (sCurTVShowSlot != -1)
        {
            TVShow *show = &gSaveBlock1Ptr->tvShows[sCurTVShowSlot];
...
static void UpdateTimeBeforeMassOutbreak(u16 days)
{
    ...
    if (gSaveBlock1Ptr->outbreakPokemonSpecies == SPECIES_NONE)
    {
        for (i = 0; i < LAST_TVSHOW_IDX; i++)
        {
            ...
            {
                if (show->massOutbreak.daysBeforeOutbreak == 0)
                    DeleteTVShowInArrayByIdx(gSaveBlock1Ptr->tvShows, i);// daily cull of inactive shows.
...
static void TryPutRandomPokeNewsOnAir(void)
{
    if (FlagGet(FLAG_SYS_GAME_CLEAR))
    {
        ...
        if (sCurTVShowSlot != -1 && RandomChance(RNG_NONE, 1, 20))
        {
...
static bool8 TryMixOutbreakTVShow(TVShow *dest, TVShow *src, u8 idx)
{
    ...
    *dest = *src;
    dest->common.active = TRUE;
    dest->massOutbreak.daysBeforeOutbreak = 0;
    return TRUE;
}
