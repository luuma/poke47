...
static bool32 IsntPyramidBag(void)
{
    return (CurrentBattlePyramidLocation() == PYRAMID_LOCATION_NONE && !FlagGet(FLAG_GAUNTLET_CHALLENGE));
}
...
bool8 ItemfinderCheckForHiddenItems(const struct MapEvents *events, u8 taskId)
{
    ...
    if (gTasks[taskId].tItemFound == TRUE || (I_ORAS_DOWSING_FLAG != 0 && gSprites[gObjectEvents[gPlayerAvatar.objectEventId].fieldEffectSpriteId].tItemFound))
        return TRUE;
    else
        return FALSE;
}
