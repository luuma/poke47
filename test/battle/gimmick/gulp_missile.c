...
SINGLE_BATTLE_TEST("POKE47: (Gulp Missile) Transformed DRIFBLIM Gulping lowers defense and still triggers other effects after")
{
    ...
    GIVEN {
        ASSUME(MoveMakesContact(MOVE_ICE_FANG));
        PLAYER(SPECIES_DRIFBLIM) { Ability(ABILITY_GULP_MISSILE); Item(ITEM_ROCKY_HELMET); }
        OPPONENT(SPECIES_DRAGAPULT) { Ability(ability); }
    } WHEN {
        TURN { MOVE(player, MOVE_STOCKPILE); MOVE(opponent, MOVE_ICE_FANG); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_STOCKPILE, player);
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_FORM_CHANGE_INSTANT, player);
        ANIMATION(ANIM_TYPE_MOVE, MOVE_ICE_FANG, opponent);
        HP_BAR(player);
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_FORM_CHANGE_INSTANT, player);
        HP_BAR(opponent);
        ...
        //ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_HELD_ITEM_EFFECT, player);//fuck you test system
        HP_BAR(opponent);
    }
}
